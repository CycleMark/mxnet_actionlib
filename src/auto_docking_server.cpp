...
//ROS_INFO_STREAM("syncCb");
  ...
  if( as_.isActive() ) {
    if ( dock_.getState() == RobotDockingState::DONE ) 
		{
			ROS_INFO("Docked DONE!");
      feedback_.state = dock_.getStateStr();
      feedback_.text = "Arrived At Docking Station Successfully";//dock_.getDebugStr();
			result_.text = "DONE...Arrived on docking station successfully.";
			result_.state = dock_.getStateStr();
      ...
      dock_.disable();
		...
		{
			ROS_INFO("Docked LOST!");
			feedback_.state = dock_.getStateStr();
      feedback_.text = "Lost";
      as_.publishFeedback(feedback_);
			result_.text = "LOST";
			result_.state = dock_.getStateStr();
			as_.setAborted( result_, "Robot LOST");
      ROS_DEBUG_STREAM( "[" << name_ << "]: Feedback sent.");
			dock_.disable();

    } 
		else if ( !dock_.isEnabled() ) 
		{ 
			ROS_INFO("Docked DISABLED!");
			//Action Server is activated, but DockDrive is not enabled, or disabled unexpectedly
